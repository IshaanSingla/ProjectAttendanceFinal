...
void teacher_menu(int TableWidth, char borderChar, char intersectionChar)
{
    ...
    case 2:
    {
        ...
        for (int i = 0; i < students.size(); i++)
        {
            ...
            if (percentage[i] < 50)
            {
                detained_clear = "DETAINED";
                flag1 = false;
            }
            printRow(std::to_string(i + 1), students[i], std::to_string(t1.getAttendedClasses(sub_choice, i + 1)) + "/" + std::to_string(t1.getTotalClasses(sub_choice)), std::to_string(percentage[i]), detained_clear, false, flag1);
        }
        printHorizontalLine(TableWidth * 2, borderChar, intersectionChar);
        int student;
